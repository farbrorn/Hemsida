...
http://balusc.blogspot.se/2009/02/fileservlet-supporting-resume-and.html
*/
...
public class ImageServerServlet extends HttpServlet {
	...
	private void cleanCache(String path, String file, String suffix) {
		...
		if (dir.isDirectory()) {
			...
			for (File f : files) {
				if (f.getName().toLowerCase().matches("\\Q" + file + ".s\\E[0-9]+\\Q" + suffix + "\\E")) {
					cn++;
					if (f.isFile()) {
						if (oldestFile==null) oldestFile = f;
						else if(f.lastModified() < oldestFile.lastModified() ) oldestFile = f;
					}
				}
			}
    ...
    private void processRequest (HttpServletRequest request, HttpServletResponse response, boolean content) throws IOException
    {
		...
		String[] split = requestedFile.split("/");
		...
		if (split.length>2) {
			if (split[1].toLowerCase().startsWith("s")) { //Det är troligen en förminskning
				try {
					...
					if (size<=0 || size > maxSize) size=null;
					else sizeString = split[1].toLowerCase();
				} catch (NumberFormatException e) {}
			}
		}
		//Ta sista biten av url:en och ignorera sökvägen
		requestedFile = URLDecoder.decode(split[split.length-1], "UTF-8");
		...
		String fileType = ".png"; //Default file type
		//Ta bort typen från filen
		if (requestedFile.toLowerCase().endsWith(".png")) { 
			requestedFile = requestedFile.substring(0, requestedFile.length()-4);
		} else if (requestedFile.toLowerCase().endsWith(".jpg")) {
			requestedFile = requestedFile.substring(0, requestedFile.length()-4);
			fileType = ".jpg";
		} 
		...
		if (size != null) {		//Om vi har omskalning
			...
			cacheFile = new File(cachePath,  requestedFile + "." + sizeString + fileType);
