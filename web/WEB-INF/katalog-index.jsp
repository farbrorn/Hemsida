...
<div class="katalog-index">
    ...
    int divBreakLevel = kgl.isAvdelningar() ? 1 : 0;
     ...
     if (kgl.isAvdelningar()) { 
        ...
        for(KatalogGrupp kg : kgl.getAvdelningar()) { %>
            <div class="katalog-index-avd"><a href="<%= request.getContextPath() + "/katalog/index/" + kg.getGrpId() %>"><%= Const.toHtml(kg.getRubrik()) %></a></div>
        <% 
            firstRun=false;
        }
    } 
    ...
    if (avdelning!=null && !avdelning.getGrpId().equals(kg.getAvdelning())) continue; 
        ...
        <div class="katalog-index-row"><h3><a href="<%= request.getContextPath()+"/katalog/"+kg.getGrpId() %>"><%= Const.toHtml(kg.getRubrik()) %></a></h3></div>
        <%  } else if(kg.getDepth() > 0) { %>
        <div class="katalog-index-row"><a href="<%= request.getContextPath()+"/katalog/"+kg.getGrpId() %>"><% for (int cnn=0; cnn<kg.getDepth()-1-divBreakLevel; cnn++ ) {%>&nbsp;&nbsp;<%} %><%=  Const.toHtml(kg.getRubrik()) %></a></div>
        <%  } %>
